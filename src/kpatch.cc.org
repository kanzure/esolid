...
{
  unsigned long i, j, k;
  unsigned long num_segments, num_endpts;
  double**      endpts;
  double        ext[4];
  unsigned long idx_ext_endpt[4];
  double        sa[4];
  double        max_aa, aa;
  unsigned long idx_max_aa_endpt;
  bool          is_ccw;
  ...
  for (i = 0, num_segments = 0; i < num_trim_curves; i++)
  {
//    for (j = 0; j < 2; j++)
//      trim_curves[i]->sub_divide(10, j);
    
    num_segments += trim_curves[i]->num_segments;
  }
  
  assert(num_segments > 0);
  num_endpts = num_segments + 1;
  
  cerr << " K_PATCH :: Bezier_output: num_segments = " << num_segments << ", num_endpts = " << num_endpts << endl << flush;
  
  endpts = new double* [num_endpts + 1];
  
  for (i = 0; i < num_endpts + 1; i++)
    endpts[i] = new double [2];
  
  trim_curves[num_trim_curves - 1]->end()->get_fp_approx(endpts[0]);
  ext[0] = ext[1] = endpts[idx_ext_endpt[0] = idx_ext_endpt[1] = 0][0];
  ext[2] = ext[3] = endpts[idx_ext_endpt[2] = idx_ext_endpt[3] = 0][1];
  
  for (i = 0, k = 1; i < num_trim_curves; i++)
  {
    for (j = 0; j < trim_curves[i]->num_segments; j++)
    {
      trim_curves[i]->segments[j]->end->get_fp_approx(endpts[k]);
      
      if (endpts[k][0] <= ext[0])
        ext[0] = endpts[idx_ext_endpt[0] = k][0];
      
      if (endpts[k][0] >= ext[1])
        ext[1] = endpts[idx_ext_endpt[1] = k][0];
      
      if (endpts[k][1] <= ext[2])
        ext[2] = endpts[idx_ext_endpt[2] = k][1];
      
      if (endpts[k][1] >= ext[3])
        ext[3] = endpts[idx_ext_endpt[3] = k][1];
  ...
  assert(k == num_endpts);
  trim_curves[0]->segments[0]->end->get_fp_approx(endpts[num_endpts]);
  
  for (i = 0; i < num_endpts; i++)
    cerr << " K_PATCH :: Bezier_output: endpt[" << i << "] = ( " << endpts[i][0] << ",  " << endpts[i][1] << " )" << endl << flush;
  cerr << endl << flush;
  cerr << " K_PATCH :: Bezier_output: min_s_endpt = " << idx_ext_endpt[0] << ", max_s_endpt = " << idx_ext_endpt[1] << ", min_t_endpt = " << idx_ext_endpt[2] << ", max_t_endpt = " << idx_ext_endpt[3] << endl << flush;
  
  assert(idx_ext_endpt[0] != idx_ext_endpt[1]);
  assert(idx_ext_endpt[2] != idx_ext_endpt[3]);
  
  if (idx_ext_endpt[0] == num_endpts - 1)
  {
    cerr << " K_PATCH :: Bezier_output: min_s_endpt may have to be shifted. " << endl << flush;
    k = 1;
    
    while (endpts[k][0] == ext[0])
      idx_ext_endpt[0] = k++;
  }
  else if (idx_ext_endpt[1] == num_endpts - 1)
  {
    cerr << " K_PATCH :: Bezier_output: max_s_endpt may have to be shifted. " << endl << flush;
    k = 1;
  ...
  if (idx_ext_endpt[2] == num_endpts - 1)
  {
    cerr << " K_PATCH :: Bezier_output: min_t_endpt may have to be shifted. " << endl << flush;
    k = 1;
    
    while (endpts[k][1] == ext[2])
      idx_ext_endpt[2] = k++;
  }
  else if (idx_ext_endpt[3] == num_endpts - 1)
  {
    cerr << " K_PATCH :: Bezier_output: max_t_endpt may have to be shifted. " << endl << flush;
    k = 1;
  ...
  for (i = 0, max_aa = 0.0, idx_max_aa_endpt = 4; i < 4; i++)
  {
    sa[i] = sgn_area(endpts[idx_ext_endpt[i] - 1],
                     endpts[idx_ext_endpt[i] + 1],
                     endpts[idx_ext_endpt[i]]);
    aa    = fabs(sa[i]);
  ...
  cerr << " K_PATCH :: Bezier_output: idx_max_aa_endpt = " << idx_max_aa_endpt << endl << flush;
  
  assert(idx_max_aa_endpt < 4);  //  => sa[idx_max_aa_endpt] != 0.0
  
  if (sa[idx_max_aa_endpt] < 0.0)       //  if x is right of x-1 -> x+1
    is_ccw = true;
  else  //  if x is left of x-1 -> x+1
    is_ccw = false;
  
  cerr << " K_PATCH :: Bezier_output: is_ccw = " << is_ccw << endl << flush;
  ...
  if (is_head)
  {
    ...
    for (i = 0; i < num_segments; i++)
    {
      ...
      out_fs << endpts[i][0] << "  " << endpts[i][1] << endl << flush;
      out_fs << endpts[i + 1][0] << "  " << endpts[i + 1][1] << endl << flush;
    }
  }
  else  //  if (!is_head)
  {
